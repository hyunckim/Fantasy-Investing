...
class Portfolio extends React.Component {
  ...
  positionsPieChart(stocks) {
    ...
    function drawChart() {
      ...
      for (let i = 0; i < stocks.length; i++) {
        data.addRow([
          stocks[i].ticker, stocks[i].number_of_shares * stocks[i].current_price
        ]);
      }
      ...
      if (document.getElementById('positions-piechart')) {
        let chart = new google
          .visualization
          .PieChart(document.getElementById('positions-piechart'));
        chart.draw(data, options);
      }
    }
  }
  ...
  render() {
    if (this.props.loading.loading && this.props.portfolio.length === 0) {
      return (
          ...
          .map((stock, idx) => {
            ...
            return (
              <tr key={idx} className='lalign'>
                <td>
                  <Link to={`company/${stock.ticker}`}>{stock.ticker}</Link>
                </td>
                <td>{stock.title}</td>
                ...
                <td>{((stock.current_price - stock.prev_close) / stock.prev_close * 100).toFixed(1)}%
                </td>
                ...
                <td>{(((stock.current_price - stock.purchase_price) / stock.purchase_price) * 100).toFixed(1)}%
                </td>
              </tr>
            );
          });
        ...
        if (percentageChange) {
          value = (
            <div className='portfolio-performance'>
              ...
              <p className={className} id="portfolio-change">
                {percentageChange.toFixed(1)}%</p>
            </div>
          );
        }
      ...
      if (this.props.currentUser && mainPortfolio) {
        ...
        if (mainPortfolio.main) {
          portChart = (
            <div id="piechart">
              {this.portfolioPieChart(totalValue - this.props.currentUser.investor.balance, this.props.currentUser.investor.balance)}
            </div>
          );
        }
        return (
